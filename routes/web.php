...
//transfer
    ...
    Route::get('/transfer/internal/otp', function(){
        return redirect(route('show_transfer_internal', session('user')['id']));
    });
    Route::post('/transfer/external/check', 'TransferController@checkOtp')->name('check_otp');
    Route::get('/transfer/statistic/{id}', 'TransferController@showStatistic')->name('show_statistic');
    Route::post('/transfer/statistic/{id}', 'TransferController@getStatistic')->name('get_statistic');
    Route::get('/transfer/report' , 'TransferController@showReport')->name('show_report');
    Route::get('/transfer/report/pdf' , 'TransferController@printReport')->name('print_report');
    Route::get('/transfer/report/back' , 'TransferController@printReport')->name('back_report');
